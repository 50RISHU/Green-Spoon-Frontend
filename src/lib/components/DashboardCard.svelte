...
<div class="glass p-4 h-100 shadow">
  <div class="text-center fs-1 mb-3">{icon}</div>
  <h4 class="text-center fw-bold">{title}</h4>
  <p class="text-muted text-center">{description}</p>
  <div class="d-grid gap-2">
    {#each actions as btn}
      <a href={btn.href} class={`btn btn-${btn.variant || 'secondary'} ${btn.outline ? 'btn-outline-' + btn.variant : ''}`}>
        {btn.text}
      </a>
    {/each}
  </div>
